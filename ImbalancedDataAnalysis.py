...
imbalancedData = DataIOFactory.getDataMatrixFromCSV("./categorizedData/im-balancedData.csv")

''' extracting feature labels and class label'''
columns_name = np.array(imbalancedData[0])#first rows has the column names
features_label =  np.delete(columns_name, [0,24], axis = None) #removing the id and class column
imbalancedData = np.delete(imbalancedData, (0), axis=0) #removing the label row from data 



'''shuffling the data'''
imbalancedData = DataIOFactory.matrixShuffling(imbalancedData)



'''Column 24, icd10 is our class and the rest are features '''
clss = imbalancedData[:, 24]
...
features =  np.delete(imbalancedData, [0,24], axis = 1)
